...
int main(int argc, char **argv) {
  ...
  parser.AddFlag('x', "extract", "Extract kFiles from archive")
      .StoreValue(opt.extract);
  ...
  parser.AddFlag('A', "concatenate", "Merge two kArchives to third archive")
      .StoreValue(opt.concatenate);
  parser.AddStringArgument('f', "file", "ArchiveWriter file")
      .StoreValue(opt.archive_filename);
  parser.AddStringArgument("kFiles", "Files as arguments")
      .Default("DEFAULT")
      .MultiValue(1)
      .Positional()
      .StoreValues(opt.filenames);
  parser.AddIntArgument('b', "blocksize", "Block size (16/32/64)")
