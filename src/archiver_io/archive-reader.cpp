...
template <size_t I, size_t O> std::string ArchiveReader<I, O>::GetName() {
  std::stringstream ss;
  char c;
...
template <size_t I, size_t O> void ArchiveReader<I, O>::ExtractFiles() {
  std::string cur_name;
  in_.seekg(0);
  std::ofstream out;
  char c;
...
template <size_t I, size_t O>
void ArchiveReader<I, O>::DeleteFiles(
    const std::vector<std::string> &filenames) {
  std::set<std::string> filenames_set{filenames.begin(), filenames.end()};
  std::string cur_name;
  in_.seekg(0);
  std::ofstream out(kTemporaryName,
                    std::ios::binary | std::ios::out | std::ios::trunc);
  char c;
...
template <size_t I, size_t O> void ArchiveReader<I, O>::Update() {
  last_block_.reset();
  block_index_ = 0;
...
template <size_t I, size_t O> char ArchiveReader<I, O>::Read() {
  ...
  for (; read_char_index_ < kByteBits; ++read_char_index_) {
    last_block_.set(block_index_++,
                    (read_char_ & (mask << read_char_index_)) != 0);
    if (block_index_ == O) {
...
template <size_t I, size_t O> char ArchiveReader<I, O>::UpdateBlock() {
  block_index_ = 0;
...
template <size_t I, size_t O> char ArchiveReader<I, O>::ReadChar() {
  for (; last_converted_block_index_ < last_converted_block_.size();
...
template <size_t I, size_t O> char ArchiveReader<I, O>::UpdateChar() {
  char res = last_char_;
  last_char_ = 0;
  char_index_ = 0;
  return res;
...
template <size_t I, size_t O> void ArchiveReader<I, O>::Close() { in_.close(); }
